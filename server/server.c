...
void *thread_func(void *thread_param) {
  ...
  while ((read_len = recv(n->connfd, recv_buf, RECV_BUF_SIZE, 0)) > 0 &&
         !exit_requested) {
    ...
    if (memchr(buffer, '\n', buf_size)) {
      ...
      syslog(LOG_INFO, "Processed %zd bytes, remaining %zu", processed,
             buf_size - processed);
...
static int create_server_socket(int *sock) {
  ...
  if (bind(*sock, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) < 0) {
    syslog(LOG_ERR, "bind() on port %d failed: %s", PORT, strerror(errno));
    close(*sock);
    return -1;
  }
...
static int create_server(int *mode) {
  ...
  if (n == NULL) {
    syslog(LOG_ERR, "failed to allocate memory for timer node: %s",
           strerror(errno));
    rc = -1;
    goto error;
  }
  ...
  while (!exit_requested) {
    ...
    n = (struct node *)calloc(1, sizeof(struct node));
    if (n == NULL) {
      syslog(LOG_ERR, "failed to allocate memory for client node: %s",
             strerror(errno));
      rc = -1;
      goto error;
    }
